...
if [ -z "$PIGMIX_DIR" ]
then 
    PIGMIX_DIR=/user/pig/pigmix/100k/
fi
...
echo "hadoop --config $conf_dir jar pigperf.jar $mainclass $hadoop_ops -libjars $zipfjar -m $mappers -r $rows -f $pages $user_field $action_field $os_field $query_term_field $ip_addr_field $timestamp_field $estimated_revenue_field $page_info_field $page_links_field"

hadoop --config $conf_dir jar pigperf.jar $mainclass $hadoop_ops -libjars $zipfjar\
...
protousers=users
echo "Skimming users"
echo "java -Xmx1024m -cp $HADOOP_CLASSPATH:$conf_dir:$PIG_HOME/pig.jar org.apache.pig.Main"
